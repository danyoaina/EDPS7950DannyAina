...
|   |   |   |   |   |   Fare ≤ 24.808
|   |   |   |   |   |   |   Age > 36.500: No {No=6, Yes=1}
|   |   |   |   |   |   |   Age ≤ 36.500: Yes {No=45, Yes=65}
|   |   |   |   Age ≤ 1.500: Yes {No=0, Yes=4}
|   |   Pclass ≤ 2.500
|   |   |   Age > 2.500
|   |   |   |   Fare > 28.856: Yes {No=1, Yes=96}
|   |   |   |   Fare ≤ 28.856
|   |   |   |   |   Age > 56: No {No=1, Yes=1}
|   |   |   |   |   Age ≤ 56: Yes {No=6, Yes=61}
...
|   |   Fare ≤ 387.665
|   |   |   Title = Master
|   |   |   |   FamilySize > 4.500: No {No=17, Yes=1}
|   |   |   |   FamilySize ≤ 4.500: Yes {No=0, Yes=17}
|   |   |   Title = Mr
|   |   |   |   Pclass > 1.500
|   |   |   |   |   Fare > 51.698
...
|   |   |   |   |   Fare > 26.144
|   |   |   |   |   |   Fare > 26.469
|   |   |   |   |   |   |   Fare > 127.817: No {No=8, Yes=0}
|   |   |   |   |   |   |   Fare ≤ 127.817
|   |   |   |   |   |   |   |   Parch > 1.500: Yes {No=1, Yes=2}
|   |   |   |   |   |   |   |   Parch ≤ 1.500: No {No=52, Yes=30}
	...
	*/
	if (model_input.gender == "Female") {
		if (model_input.familysize > 7.5) {
			return { prediction: "Died", likelihood: "100%", series_survived: 0, series_died: 5 };
		} else {
			if (model_input.class > 2.5) {
				if (model_input.fare > 32.881) {
					return { prediction: "Died", likelihood: "100%", series_survived: 0, series_died: 4 };
				} else {
					if (model_input.age > 1.5) {
						if (model_input.fare > 31.331) {
							return { prediction: "Died", likelihood: "100%", series_survived: 0, series_died: 2 };
						} else {
							if (model_input.fare > 24.808) {
								return { prediction: "Died", likelihood: "100%", series_survived: 0, series_died: 12 };
							} else {
								if (model_input.fare > 36.500) {
											return { prediction: "Survived", likelihood: "14%", series_survived: 1, series_died: 6 };
										} else {
											return { prediction: "Died", likelihood: "86", series_survived: 1, series_died: 6 };
										}
									} else {
										return { prediction: "Survived", likelihood: "100%", series_survived: 100, series_died: 0 };
									} {
									return { prediction: "Survived", likelihood: "71%", series_survived: 5, series_died: 2 };
								} else {
									return { prediction: "Died", likelihood: "66%", series_survived: 4, series_died: 3 };
								}
							}
						}
					} else {
						return { prediction: "Survived", likelihood: "100%", series_survived: 7, series_died: 0 };
					}
				}
			} else {
				return { prediction: "Survived", likelihood: "", series_survived: 6, series_died: 1 };
			}
		}
	} else {
				...
				if (model_input.age > 3.5) {
					if (model_input.age > 9.5) {
						if (model_input.title == "Master") {
							return { prediction: "Died", likelihood: "94%", series_survived: 1, series_died: 17 };
